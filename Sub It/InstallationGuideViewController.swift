...
//        getIt.isBrewInstalled = false
    ...
    @IBAction func openTerminalButtonClicked(_ sender: Any) {
        _ = subIt.execute(commandSynchronous: "open /Applications/Utilities/Terminal.app")
    }
    ...
    @IBAction func nextPageBtnClicked(_ sender: Any) {
        ...
        if(currentPage == 0 && subIt.isXcodeInstalled && subIt.isBrewInstalled && subIt.isPythonInstalled) { currentPage = 4 }
        else if(currentPage == 0 && subIt.isXcodeInstalled && subIt.isBrewInstalled) { currentPage = 3 }
        else if(currentPage == 0 && subIt.isXcodeInstalled) { currentPage = 2 }
        else if(currentPage == 1 && subIt.isBrewInstalled && subIt.isPythonInstalled) { currentPage = 4 }
        else if(currentPage == 1 && subIt.isBrewInstalled) { currentPage = 3 }
        else if(currentPage == 2 && subIt.isPythonInstalled) { currentPage = 4 }
    ...
    @IBAction func previousPageBtnClicked(_ sender: Any) {
        ...
        if(currentPage == 4 && subIt.isXcodeInstalled && subIt.isBrewInstalled && subIt.isPythonInstalled) { currentPage = 0 }
        else if(currentPage == 4 && subIt.isPythonInstalled && subIt.isBrewInstalled) { currentPage = 1 }
        else if(currentPage == 4 && subIt.isPythonInstalled) { currentPage = 2 }
        else if(currentPage == 3 && subIt.isXcodeInstalled && subIt.isBrewInstalled) { currentPage = 0 }
        else if(currentPage == 3 && subIt.isBrewInstalled) { currentPage = 1 }
        else if(currentPage == 2 && subIt.isXcodeInstalled) { currentPage = 0 }
        else{ currentPage = currentPage - 1 }
